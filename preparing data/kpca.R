...
kpca = kpca(~., data = training_set[-27], kernel = 'rbfdot',features = 2)

##test

training_set_kpca = as.data.frame(predict(kpca, training_set))
training_set_kpca$result = training_set$result 

test_set_kpca = as.data.frame(predict(kpca, test_set))
test_set_kpca$result = test_set$result

svm_Radial = svm(formula = result ~., 
